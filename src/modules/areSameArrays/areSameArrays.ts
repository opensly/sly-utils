...
export const areSameArrays = (arr1: any[], arr2: any[]): boolean => {
  ...
  const frequencyMap1 = arr1.reduce((map: Record<string, number>, item: any) => {
    const key = typeof item + ':' + item;
    map[key] = (map[key] || 0) + 1;
    return map;
  }, {} as Record<string, number>);

  const frequencyMap2 = arr2.reduce((map: Record<string, number>, item: any) => {
    const key = typeof item + ':' + item;
    map[key] = (map[key] || 0) + 1;
    return map;
  }, {} as Record<string, number>);
