...
export const compareStrings = (source: string, target: string): number => {
  const cleanString = (str: string): string => str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  ...
  const diff: number[][] = Array.from({ length: n + 1 }, (_, i) => Array(m + 1).fill(0));
